...
// core.Invocation. A port of ../../../current/go/main.go, which drives
// Nightseam v0.6.0's runtime.Invocation with the same operations. Expectations
// are read only by Bitwire's runner; this driver has no replacement ledger or
// routing machinery.
package main
...
func observe(test testCase) []any {
	retirements := 0
	controls := []string{}
	newInvocation := func() *core.Invocation {
		return core.NewInvocation(test.Limits, func() { retirements++ })
	}
	invocation := newInvocation()
	// An opaque send-only facade: participants cannot discover the Invocation.
	facade := func(v *core.Invocation) wire.AddressedWire { return sending(v.Deliver) }
	access := facade(invocation)
	var previous wire.AddressedWire
	rows := []any{}
