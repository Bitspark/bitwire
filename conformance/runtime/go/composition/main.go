// Exercises bitruntime's carriers against Bitwire's six composition
// observation groups (../../../reference/expected.json). A port of Nightseam's
// upstream driver at v0.6.0 (5cc9723a, conformance/bitwire/go/main.go; Apache-2.0,
// Bitspark), with each Nightseam facility replaced by the bitruntime facility
// its port records. It contains no replacement Wire implementation and no
// expected results; Bitwire's runner compares the observations.
package main
...
func pair() (bitwire.Endpoint, bitwire.Endpoint) {
	return carrier.Pair(func(release func()) { cleanups = append(cleanups, release) })
}
...
func closeWire(wire interface {
	...
	check(wire.Close(transports.CodeNormal, "conformance complete"))
}
...
func dispatcher(e bitwire.Endpoint) *dispatch.Dispatcher {
	d, err := dispatch.NewDispatcher(e)
	check(err)
	cleanups = append(cleanups, func() { closeWire(d) })
	return d
}
func send(w bitwire.AddressedWire, path []string, done <-chan struct{}) {
...
func siblings() any {
	...
	send(core.At(client, []string{"a"}), []string{"run"}, done)
	send(core.At(client, []string{"b"}), []string{"run"}, done)
	detachA()
	detachA()
	check(core.At(client, []string{"a"}).Send([]string{"ignored"}, event()))
	send(core.At(client, []string{"b"}), []string{"after-detach"}, done)
	closeWire(router)
...
func composition() any {
	...
	mounted := core.Mount(map[string]bitwire.Endpoint{"": callerRouter.Select([]string{"a"}).Select([]string{"b"})})
	defer closeWire(mounted)
	composed := core.At(mounted, []string{""})
...
func sameIDDelayedReplies() any {
	...
	for _, entry := range []struct{ name, payload string }{{"left", "first"}, {"right", "second"}} {
		...
		check(err)
		check(core.At(client, []string{"service"}).Send([]string{"call"}, bitwire.Message{Frame: bitwire.ProfileFrame{Version: 1, Kind: bitwire.ProfileRequest, ID: "c:1", Params: payload}, Return: &bitwire.ReturnAddress{Wire: sink}}))
		wait(done)
	}
	stop()
	closeWire(view)
