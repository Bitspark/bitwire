// Records public invocation lifecycle observations from bitruntime's
// Invocation. A port of ../../current/ts/lifecycle.ts, which drives nightseam
// v0.6.0's Invocation with the same operations. Expectations are read only by
// bitwire's runner; this driver has no replacement ledger or routing machinery.
import { readFileSync } from 'node:fs';
import type { AddressedWire, Message } from '@bitspark/bitwire';
import { Invocation, type InvocationLimits } from '@bitspark/bitruntime/core';

interface Step { op: string; id?: string; label?: string }
interface Case { id: string; limits: InvocationLimits; steps: Step[] }
...
function observe(test: Case): unknown[] {
  ...
  const facade = (owner: Invocation): AddressedWire => ({ send: (path, message) => owner.deliver(path, message) });
  let access = facade(invocation);
  let previous: AddressedWire | undefined;
  const rows: unknown[] = [];
...
if (process.argv.length !== 3) throw new Error('Usage: lifecycle.ts inputs.json');
