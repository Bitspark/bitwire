// Test-only ADR0005 interpreter. Released endpoints own dispatch and lifecycle;
// this file does not establish a production declared-composition API.
import { readFileSync } from 'node:fs';
import { once } from 'node:events';
import { isDeepStrictEqual } from 'node:util';
import { WebSocket, WebSocketServer } from 'ws';
import type { Endpoint, Message, Path, ReturnAddress, Wire } from '@bitspark/bitwire';
import { at, mount, encodePath, webSocketConnection, type WebSocketLike } from '@nightseam/duplex';
import { wirePair, forwardWire, DuplexPeer } from '@nightseam/runtime';

type Check = [string, string[]];
interface Policy { id: string; remaining: number; checks: Check[] }
interface Declaration { readonly own: Wire; readonly policy?: Policy; readonly children: ReadonlyMap<string, Declaration> }
type Parts = [Wire, Policy | undefined, [string, Declaration][]];
const refuse: Wire = { send() { throw new Error('No origin'); } };
function compose(own: Wire, policy: Policy | undefined, entries: readonly (readonly [string, Declaration])[]): Declaration {
  const children = new Map<string, Declaration>();
  for (const [key, child] of entries) {
    encodePath([key]);
    if (!child || children.has(key)) throw new Error('Missing or conflicting child');
    children.set(key, child);
  }
  return { own, policy, children };
}
function parts(d: Declaration): Parts { return [d.own, d.policy, [...d.children]]; }
function rebuild(d: Declaration, deep: boolean, memo = new Map<Declaration, Declaration>()): Declaration {
  ...
  const next = compose(own, policy, deep ? children.map(([k, child]) => [k, rebuild(child, true, memo)] as const) : children);
  memo.set(d, next);
  return next;
}
function retained(d: Declaration): boolean {
  const p = parts(d);
  const other = compose(...p);
  const same = d.own === other.own && d.policy === other.policy && d.children.size === other.children.size
    && [...d.children].every(([k, v]) => other.children.get(k) === v);
  p[2].length = 0;
  return same && d.children.size === other.children.size;
}
function dispatch(d: Declaration, path: Path, message: Message): void {
  if (d.policy) {
    const g = d.policy;
    g.checks.push([g.id, [...path]]);
    if (g.remaining === 0) throw new Error('Policy refused');
    if (g.remaining > 0) g.remaining--;
  }
  if (path.length === 0) return d.own.send([], message);
  const child = d.children.get(path[0]!);
  if (!child) throw new Error('No child');
  dispatch(child, path.slice(1), message);
}
function bind(d: Declaration): Wire {
  return { send(path, message) {
    encodePath(path);
    if (message.frame.kind !== 'request' && message.frame.kind !== 'event') throw new Error('Not a new admission');
    dispatch(d, path, message);
  } };
}
interface NodeSpec { id: string; own: string | null; policy?: string; children: [string, string][] }
interface Step { op: string; path?: string[]; selections?: string[][]; raw?: string; cut?: string; kind?: string; useView?: boolean }
interface Case { id: string; kind?: string; fault?: string; limits: Record<string, number>; steps: Step[]; relay?: boolean; mount?: boolean }
function create(specs: NodeSpec[], test: Case, own: (name: string) => Wire, checks: Check[]): [Declaration, Map<string, Declaration>] {
  ...
  const gates = new Map(Object.entries(test.limits).map(([id, remaining]) => [id, { id, remaining, checks }]));
  ...
  function build(id: string): Declaration {
    ...
    const next = compose(spec.own === null ? refuse : own(spec.own), spec.policy ? gates.get(spec.policy) : undefined, entries);
    visiting.delete(id);
    nodes.set(id, next);
    return next;
  }
...
class Scope {
  ...
  async pair(): Promise<[Endpoint, Endpoint]> {
    if (process.env.NIGHTSEAM_BITWIRE_CARRIER === 'local') {
      const pair = wirePair();
      this.cleanups.push(() => { pair[0].close(); pair[1].close(); });
      return pair;
    }
    const listener = new WebSocketServer({ port: 0, host: '127.0.0.1' });
    this.cleanups.push(() => { listener.close(); });
    ...
    const a = new DuplexPeer({ role: 'client' }), b = new DuplexPeer({ role: 'server' });
    this.cleanups.push(() => { a.close(); b.close(); });
...
interface Delivery { path: Path; message: Message; count: number }
...
async function observe(specs: NodeSpec[], test: Case): Promise<unknown> {
  ...
  try {
    ...
    const deliveries = mailbox<Delivery>(), counts = new Map<string, number>();
    scope.cleanups.push(receiver.receive({ message(path, message) {
      if (path.length !== 1) throw new Error('Unexpected destination path');
      const name = path[0]!, count = (counts.get(name) ?? 0) + 1;
      counts.set(name, count);
      deliveries.put({ path: [...path], message, count });
    } }));
    const checks: Check[] = [];
    let unchanged = true, expected: Message;
    const marker = {}, contexts = new Map<ReturnAddress, object>(), origins = new Map<string, Wire>();
    const own = (name: string): Wire => {
      const existing = origins.get(name);
      if (existing) return existing;
      const origin: Wire = { send(path, message) {
        unchanged &&= path.length === 0 && isDeepStrictEqual(message.frame, expected.frame)
          && message.return === expected.return && !!message.return && contexts.get(message.return) === marker;
    ...
    try { [root, nodes] = create(specs, test, own, checks); }
    catch { return { construction: 'refused' }; }
    let partsRetained = retained(root);
    let selected: Wire | undefined;
    const outcomes: string[] = [], observed: [string, number][] = [];
    for (const [index, action] of test.steps.entries()) {
      switch (action.op) {
        ...
        case 'rebind': {
          ...
          root = compose(origin, policy, children.map(([k, v]) => [k,
            k === 'a' ? compose(v.own, v.policy, [['b', compose(own('replacement'), undefined, [])]]) : v]));
          break;
        }
        case 'rebuild':
        ...
        case 'substitute': {
          const [origin, policy, children] = parts(root);
          root = compose(origin, policy, children.map(([k, v]) => [k, k === 'a' ? rebuild(v, true) : v]));
          break;
        }
        case 'boundChild': {
          const selected = at(bind(root), ['a', 'b']);
          root = compose(root.own, root.policy, [['a', compose(selected, undefined, [])]]);
          break;
        }
        case 'resetPolicy': {
          const [origin, , children] = parts(root);
          root = compose(origin, { id: 'outer', remaining: test.limits.outer!, checks }, children);
          break;
        }
        case 'sharePolicy': {
          const [origin, policy, children] = parts(root);
          root = compose(origin, policy, children.map(([k, v]) => [k, k === 'a' ? compose(v.own, policy, [...v.children]) : v]));
          break;
        }
        case 'dropOwn': root = compose(refuse, root.policy, [...root.children]); break;
        case 'omit':
        case 'rename':
        case 'extra': {
          const [origin, policy, children] = parts(root);
        ...
        case 'control': {
          ...
          if (action.op === 'control') expected = {
            frame: action.kind === 'cancel' ? { version: 1, kind: 'cancel', id: 'c:1' }
              : { version: 1, kind: 'response', id: 'c:1', result: null },
            return: expected.return,
          };
          const path = action.op === 'invalidPath' ? [String.fromCharCode(0xd800)] : action.path!;
          let admitted = true;
          try { access.send(path, expected); } catch { admitted = false; }
          outcomes.push(admitted ? 'admitted' : 'refused');
          if (admitted) {
            const got = await deliveries.take();
            if (got.message.frame.kind !== 'event' || expected.frame.kind !== 'event' || got.message.frame.data !== expected.frame.data) {
        ...
        default: throw new Error('Unknown step: ' + action.op);
      }
    }
    const sendOnly = Object.keys(bind(root)).join(',') === 'send';
    mounted?.close();
    return { outcomes, checks, deliveries: observed, unchanged, sendOnly, partsRetained, borrowedUsable: await borrowed(source, deliveries) };
  } finally { scope.close(); }
}
...
async function pending(specs: NodeSpec[], test: Case): Promise<unknown> {
  ...
  try {
    ...
    const captured = mailbox<Message>(), cancelled = mailbox<string>(), deliveries = mailbox<Delivery>(), replies = mailbox<string>();
    scope.cleanups.push(receiver.receive({ message(path, message) {
      if (message.frame.kind === 'event') { deliveries.put({ path, message, count: 1 }); return; }
    ...
    let [root] = create(specs, test, name => ({ send(path, message) {
      preserved &&= message.return === original && path.length === 0;
      at(sender, [name]).send(path, message);
    } }), checks);
    at(at(bind(root), ['a']), ['b']).send([], { frame: { version: 1, kind: 'request', id: 'c:1', params: null }, return: original });
    ...
    root = compose(root.own, root.policy, [['alias', replacement], ['a', replacement]]);
    let newCallRefused = false;
    try { bind(root).send(['alias'], event('new')); } catch { newCallRefused = true; }
    ...
    return { checks, newCallRefused, lateReply, cancelled: controls, returnPreserved: preserved, borrowedUsable: await borrowed(sender, deliveries) };
  } finally { scope.close(); }
}
if (!['local', 'peer'].includes(process.env.NIGHTSEAM_BITWIRE_CARRIER ?? '')) throw new Error('Expected local or peer carrier');
const fixture = JSON.parse(readFileSync(process.argv[2]!, 'utf8')) as { nodes: NodeSpec[]; cases: Case[] };
const output = [];
for (const test of fixture.cases) output.push({
  id: test.id, observations: await (test.kind === 'pending' ? pending(fixture.nodes, test) : observe(fixture.nodes, test)),
});
