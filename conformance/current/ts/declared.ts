...
const refuse: Origin = { name: '', instance: 0, handle() { throw new Error('No destination at the origin'); } };
...
class Reference implements Realization {
  private readonly retained = new Map<Wire, { own: Origin; children: ReadonlyMap<string, Wire> }>();
  compose(own: Origin, entries: readonly Entry[]): Wire {
    ...
    const send = (path: Path, message: Message): void => {
      ...
      if (!child) throw new Error('No destination');
      child.send(path.slice(1), message);
    };
    const access: Wire = { send(path, message) {
      if (!path.every(wellFormed)) throw new Error('Invalid path');
      send(path, message);
    } };
...
class Production implements Realization {
  ...
  expose(w: Wire): Wire { return at(w, []); }
  teardown(): void { for (const mounted of this.mounts) mounted.close(1000, 'released'); }
}
...
class Env {
  ...
  constructor(sender: Wire) { this.sender = sender; }
...
class Guard implements Wire {
  ...
  readonly inner: Wire;
  private readonly env: Env;
  constructor(policy: Policy, inner: Wire, env: Env) { this.policy = policy; this.inner = inner; this.env = env; }
  send(path: Path, message: Message): void {
...
class Harness {
  ...
  caller(): Wire { return this.root instanceof Guard ? this.root : this.R.expose(this.root); }
...
class Scope {
  ...
  async carriers(test: Case): Promise<[Endpoint, Wire, Endpoint]> {
    ...
    let sender: Wire = source, receiver = first;
    if (test.mount) {
...
const refuseWire: Wire = { send() { throw new Error('No destination'); } };
...
async function apply(h: Harness, test: Case, index: number, s: Step, deliveries: Deliveries): Promise<void> {
  const edit = (f: (own: Origin, entries: Entry[]) => Wire) => (w: Wire): Wire => {
