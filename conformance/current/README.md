...
| Declared composites, reference | Bitwire's [fixtures](../declared/README.md) through test-only interpreters of decision 0006, using released endpoints, selection, forwarding and invocation facilities | 39 cases per language on local pairs and WebSockets in both directions. Origins and complete children, complete cuts, empty keys, missing and altered children, shared state, substitution, guards, forwarding, teardown and a pending reply/cancellation after reconstruction. Interpreter evidence, not a released construction API. |
| Declared composites, production | The same fixtures through released `Mount`/`mount`, `At`/`at` and forwarding | The 20 child-only cases conform on every carrier. The other 19 must match the exact observations in the [gap ledger](../declared/production-gaps.json): no origin-bearing constructor, and conflicting, invalid or missing children accepted at construction. |
