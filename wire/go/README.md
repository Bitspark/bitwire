...
origin. Segments map to Deixis keys by exact UTF-8 encoding. A Go `string`
segment is a key only when it is valid UTF-8. Rebuilding uses parts retained by
the construction owner; a send-only Wire reveals no structure. The [shared
