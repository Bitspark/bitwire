...
origin. Segments map to Deixis keys by exact UTF-8 encoding. A `std::string`
segment is a key only when it holds valid UTF-8. Rebuilding uses parts retained
by the construction owner; a send-only Wire cannot enumerate or unwrap them,
though its behavior may reveal which routes respond. The [shared
